...
class GCounter:
    ...
    def merge(self, packet):
        for key in packet:
            ...
            if key in self.counter:
                self.counter[key].count = max(self.counter[key].count, otherCount)
            else:
    ...
    def ping(self):
        packet = dict()
        for key in self.counter:
            packet[key] = str(self.counter[key].count) + "," + self.counter[key].host
        for host in self.allHosts.split(","):
            if host != self.counter[self.id].host:
                requests.post("http://" + host + "/merge", json=json.dumps(packet))
